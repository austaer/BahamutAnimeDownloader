...
func (h *bahamut) getTitle() {
	doc, err := goquery.NewDocument("https://ani.gamer.com.tw/animeVideo.php?sn=" + h.sn)
	isErr("Create request failed - ", err)

	h.title = doc.Find(".anime_name h1").Text()
	fmt.Println("Anime name: " + h.title)
}
